...
COIN_LATEST_RELEASE='linux.Ubuntu.16.04.LTS-static-libstdc.tar.gz'
COIN_TGZ=$(curl -s https://api.github.com/repos/LIMXTEC/Bitcloud/releases/latest | grep -i $COIN_LATEST_RELEASE | grep -i "browser_download_url" | awk -F" " '{print $2}' | sed 's/"//g')
...
wget -q $COIN_CHAIN
tar -xzvf blockchain.tar.gz -C /root/btdx_temp >/dev/null 2>&1
